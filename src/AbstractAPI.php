    ...
    protected function startAndLoopInternal(string $eventHandler): void
    {
        ...
        EventLoop::setErrorHandler(
            $c = function (\Throwable $e) use (&$errors, &$started): void {
                ...
                if (method_exists($e, 'getError') && $e->getError() instanceof VKApiError) {
                    $this->wrapper->getAPI()->logger->logger($e->getMessage().', exiting!', Logger::LEVEL_ERROR);
                    throw $e;
                }
                ...
                $errors[$time]++;
                if ($errors[$time] > 5 && (!$started || !$this->wrapper->getAPI()->isInit())) {
                    $this->wrapper->getAPI()->logger->logger(
                        'More than 10 errors per second, exiting!',
                        Logger::FATAL_ERROR
                    );
                    return;
                }
